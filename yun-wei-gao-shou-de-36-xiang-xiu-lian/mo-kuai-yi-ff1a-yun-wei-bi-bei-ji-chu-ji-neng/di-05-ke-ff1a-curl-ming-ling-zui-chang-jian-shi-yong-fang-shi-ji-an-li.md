...
![](/static/image/Cgq2xl5eIS6AOfjaAAIwCTPcHBc162.png)
...
通过 -v 参数\(curl -v www.jesonc.com\)可以看到服务端返回的状态码为 301，代表返回了一个重定向的状态码，并且会给到 location 的 head 头信息。我们知道 301 的状态码表示直接把请求重定向到另外一个地址， location 后的这个地址就是重定向的地址（Location: [http://www.jesonc.com/jeson）。这个时候](http://www.jesonc.com/jeson）。这个时候) Curl 命令中有没有可能一步到位去请求这个地址，就这个时候就可以通过 -L 参数来实现。

这里我加一个 -L 参数（curl -L [http://www.jesonc.com）来看下效果。这时我们会看到整个通信过程就变化了，然后有很多](http://www.jesonc.com）来看下效果。这时我们会看到整个通信过程就变化了，然后有很多) body 的信息打印出来，我们回到打印信息的最上面，去看一下通信过程。
...
所以接下来，我在 Curl 请求中添加一个 Referer 头请求，这里加入一个 -e 选项（curl -e "[http://www.jesonc.com/](http://www.jesonc.com/)" -I [http://www.jesonc.com/download/ywgs\_lg.tgz](http://www.jesonc.com/download/ywgs_lg.tgz)
...
![](/static/image/CgpOIF5eJPuAcTrFAAFf3jurHwE260.png)
...
![](/static/image/CgpOIF5eJTiAcOqIAAC2MiN5M3Q570.png)
